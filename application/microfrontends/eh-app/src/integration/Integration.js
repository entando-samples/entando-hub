...
export const createAUserForAnOrganisation = async (
  ...
  if (type === 'update') {
    eventHandler(isError,
      `${i18n.t('toasterMessage.impossibleToCreateUser')}: ${data?.message || ''}`,
...
export const deleteUser = async (apiUrl,username) => {
  ...
  eventHandler(isError, `${i18n.t('toasterMessage.impossibleToDeleteUser')}: ${data?.message || ''}`, `User deleted`)
...
export const removeUserFromOrganisation = async (apiUrl,organisationId, username, type) => {
  ...
  if (type === 'update') {
    eventHandler(isError, `${i18n.t('toasterMessage.impossibleToRemoveUser')}: ${data?.message || ''}`, ``);
  } else {
    eventHandler(isError, `${i18n.t('toasterMessage.impossibleToRemoveUser')}: ${data?.message || ''}`, `${i18n.t('toasterMessage.userRemovedFromTheOrganisation')}`);
  }
 ...
 */
 export const getBundleGroupDetailsByBundleGroupVersionId = async (apiUrl, bundleGroupVersionId, params = {}) => {
  ...
  eventHandler(isError, `${i18n.t('toasterMessage.impossibleToLoadBundleGroup')}: ${data?.message || ''}`)
